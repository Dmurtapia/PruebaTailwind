...
<input
    type="text"
    id="searchTicket"
    placeholder="Buscar por código (CT-XXXXXX)"
...
@forelse($tickets as $email => $userTickets)
    @foreach($userTickets as $ticket)
        ...
        <div class="ai-card mb-4 space-y-2 ticket-card"
             data-code="{{ strtoupper($ticket['codigo']) }}">
            ...
            <div class="flex gap-3 mt-4">
                ...
                @if($ticket['estado'] !== 'Completado')
                    <form method="POST"
                          action="{{ route('admin.ticket.completar', [$email, $ticket['id']]) }}">
                        @csrf
                ...
                <form method="POST"
                      action="{{ route('ticket.borrar', [$email, $ticket['id']]) }}">
                    @csrf
